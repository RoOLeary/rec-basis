...
const teams = [
    {
      ...
      team_description: 'Building the perfect assortment',
      team_link: '/commercial',
     ...
     <div className="relative mx-auto w-full px-6 md:w-3/4 sm:px-8 lg:px-8 py-4">
        ...
        >
          ...
          {teams.map((team) => (
            <li
              key={team.team_name}
              className="shortCut text-center"
            >
              <Link href={`${team.team_link.toLowerCase()}`}>
              <h3 className="font-bold text-gray-900 ">
                {team.team_name}
              </h3>
              <p className="text-gray-700">{team.team_description}</p>
              </Link>
            </li>
          ))}
        </ul>
        <div className="mx-auto flex mt-8 justify-center">
          <Link className="justify-center rounded-lg py-2 px-3 text-sm font-semibold outline-2 outline-offset-2 transition-colors bg-red-600 text-white hover:bg-red-600 active:bg-red-600 active:text-white/80 block w-32 mb-6
            text-center shake" href="/jobs"><span>All Jobs</span>
          </Link>
        </div>
